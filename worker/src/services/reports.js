// שירות דוחות - אגרגציות ושאילתות סטטיסטיות על המשלוחים (D1).
// כל פונקציה מחזירה אובייקט עם summary (סיכומים) ושורות הפירוט.
 ...
 */
export async function getMonthlyReport(db, year, month) {
 ...
 */
export async function getMismatchesReport(db, from, to) {
  const { results: shipments } = await db
    .prepare(
 ...
 */
export async function getActiveShipmentsReport(db) {
  const { results: shipments } = await db
    .prepare(
 ...
 */
export async function getBranchActivityReport(db, branchId, from, to) {
  const branch = await db.prepare('SELECT * FROM branches WHERE id = ?').bind(branchId).first();
  if (!branch) {
    throw httpError('הסניף לא נמצא', 404);
  }
